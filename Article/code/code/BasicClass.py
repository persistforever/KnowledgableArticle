...
class Article :
    ...
    # ----- features -----
    # --- 1. token level ---
    contentlength = 0 # length of content
    titlelength = 0 # length of title
    wordnumct = 0 # number of distinct word in content
    wordnumtl = 0 # number of distinct word in title
    distwordnumct = 0 # number of distinct word in content
    distwordnumtl = 0 # nubmer of distinct word in title
    puncnumct = 0 # number of punctuation in content
    puncnumtl = 0 # number of punctuation in title
    sentencenum = 0 # number of sentence
    avgstlength = 0 # average length of sentence
    paragraphnum = 0 # number of paragraph
    avgpgstnum = 0 # average lenght of paragraph
    # --- 2. user level ---
    ...
    namenumct = 0 # number of name word in content
    timenumct = 0 # number of time word in content
    placenumct = 0 # number of place word in content
    klwordnum = 0 # number of knowledgable word
    namenumtl = 0 # number of name word in title
    timenumtl = 0 # number of time word in title
    placenumtl = 0 # number of place word in title
    # --- 4. pos level ---
    ...
    def importSplitTitle(self, data) :
        self.sptitle, self.spcontent = [], []
        if len(data) >= 3 :
            title = data[1]
            for part in title.split(' ') :
    ...
    def constrFeatureSet(self) :
        self.featureset = []
        self.featureset.extend([self.titlelength, self.contentlength, self.wordnumtl, self.wordnumct, \
                self.distwordnumtl, self.distwordnumct, self.puncnumtl, self.puncnumct, \
                self.sentencenum, self.avgstlength, self.paragraphnum, self.avgpgstnum])
        self.featureset.extend([self.forwardnum, self.zannum, self.clicknum, self.collectnum, self.readtime, self.finishrate])
        self.featureset.extend([self.firpronum, self.secpronum, self.thrpronum, \
                self.namenumtl, self.namenumct, self.timenumtl, self.timenumct, \
                self.placenumtl, self.placenumct, self.klwordnum])
